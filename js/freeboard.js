		...
		_.each(settingsDefs, function (settingDef) {
			if (settingDef.type == "calculated") {
				...
				if (!_.isUndefined(script)) {
					...
					function make_function(code) {
						// If there is no return, add one.
						// TODO This needs a proper method to decide if the code is an expression or more.
						if ((code.match(/;/g) || []).length <= 1 && code.indexOf("return") == -1) {
							code = "return " + code;
						}
						...
						try {
							valueFunction = new Function("datasources", code);
						}
						catch (e) {
							console.log("Error makeing this into a function:");
							console.log(code);
							console.log(e.toString());
					...
					if(_.isArray(script)) {
						script = _.map(script, make_function);
						valueFunction = make_function("return [(" + script.join(")(datasources),(") + "(datasources))];");
					} else {
						valueFunction = make_function(script);
					}
